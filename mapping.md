...
wezterm has windows, windows have tabs, tabs have panes, panes have one command
nvim has tabs, which have panes, which have buffers
should configure them similarly
also see screen/tmux
...
-----------------
	...
	SHIFT | SUPER        PageUp             ->   MoveTabRelative(-1)
	SHIFT                PageDown           ->   ScrollByPage(NotNan(1.0))
	CTRL                 PageDown           ->   ActivateTabRelative(1)
	SHIFT | SUPER        PageDown           ->   MoveTabRelative(1)
...
-----
	...
	SUPER          Drag { streak: 1, button: Left }           ->   StartWindowDrag
	               ...
	               Up { streak: 1, button: Left }             ->   CompleteSelectionOrOpenLinkAtMouseCursor(PrimarySelection)
	SHIFT          Up { streak: 1, button: Left }             ->   CompleteSelectionOrOpenLinkAtMouseCursor(PrimarySelection)
	ALT            Up { streak: 1, button: Left }             ->   CompleteSelection(PrimarySelection)
	SHIFT | ALT    Up { streak: 1, button: Left }             ->   CompleteSelectionOrOpenLinkAtMouseCursor(PrimarySelection)
	               Up { streak: 2, button: Left }             ->   CompleteSelection(PrimarySelection)
	               Up { streak: 3, button: Left }             ->   CompleteSelection(PrimarySelection)
